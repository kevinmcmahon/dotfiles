#!/bin/bash
# ABOUTME: Single-line Claude Code statusline with visual context progress bar.
# ABOUTME: Shows model, folder, branch, context usage, cost, and session duration.
#
...
# Single line: [Model] folder | branch | progress bar context% | $cost | duration | cache%
output=$(printf '\033[37m[%s]\033[0m' "$short_model")
output="$output $(printf '\033[94m📁 %s\033[0m' "$folder_name")"
if [ -n "$git_branch" ]; then
  output="$output $(printf '%b \033[96m🌿 %s\033[0m' "$SEP" "$git_branch")"
fi
if [ -n "$progress_bar" ]; then
  output="$output $(printf '%b %b' "$SEP" "$progress_bar")"
fi
if [ -n "$ctx_pct" ]; then
