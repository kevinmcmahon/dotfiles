...
Existing files are backed up with a `.bak.<timestamp>` suffix before being replaced. Generated AI resource sync is more conservative: `scripts/ai-sync.sh` skips non-symlink conflicts and only removes stale symlinks that point back into `ai/`.
